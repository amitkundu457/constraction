...
class VenderController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (\Auth::user()->can('create vender')) {
            ...
            if ($total_vendor < $plan->max_venders || $plan->max_venders == -1) {
                ...
                $vender->client_id             = $request->client_id;
    ...
    {
        // $client = User::join('contracts', 'contracts.client_name', '=', 'users.id')
        //     ->select('users.id','users.email', 'contracts.start_date', 'contracts.end_date', 'contracts.value')->where('users.id', $id)->first();
        $client = User::join('contracts', 'contracts.client_name', '=', 'users.id')
            ->select('users.id','users.email', 'contracts.value')->where('users.id', $id)->first();
