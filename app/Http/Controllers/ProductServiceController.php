...
class ProductServiceController extends Controller
{
    ...
    public function create()
    {
        if (\Auth::user()->can('create product & service')) {
            ...
            $company = Company::where('status', 1)->pluck('name', 'id')->toArray();
            $vender = Vender::all()->pluck('name', 'id')->toArray();
            return view('productservice.create', compact('category', 'unit', 'tax', 'customFields', 'incomeChartAccounts', 'expenseChartAccounts', 'company', 'vender'));
        } else {
