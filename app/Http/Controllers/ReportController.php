...
class ReportController extends Controller
{
    ...
    public function purchaseDailyReport(Request $request)
    {
        if (\Auth::user()->can('manage pos')) {
            ...
            $purchases = $query->get()->groupBy(function ($val) {
            ...
            $query = Purchase::join('purchase_products as pp', 'pp.purchase_id', '=', 'purchases.id')
                ->join('product_services as p', 'pp.product_id', '=', 'p.id')
                ->join('companies', 'purchases.company_id', '=', 'companies.id')
                ->join('venders', 'purchases.vender_id', '=', 'venders.id')
                ->select('purchases.id', 'pp.price', 'pp.quantity', 'p.name', 'purchases.purchase_date', 'pp.price', 'companies.name as cname', 'venders.name as vname')
                ->whereBetween('purchases.created_at', [$startDate, $endDate]);

            // Conditionally apply the vendor filter if a vendor ID is provided
            if ($vendorId) {
                $query->where('purchases.vender_id', $vendorId);
            }

            // Execute the query and get results
            $purdata = $query->get();

            // $purdatatotal = Purchase::join('purchase_products as pp', 'pp.purchase_id', '=', 'purchases.id')
            //     ->join('product_services as p', 'pp.product_id', '=', 'p.id')
            //     ->join('companies', 'purchases.company_id', '=', 'companies.id')
            //     ->join('venders', 'purchases.vender_id', '=', 'venders.id')
            //     ->select('purchases.id', 'pp.price', 'pp.quantity', 'p.name', 'purchases.purchase_date', 'pp.price', 'companies.name as cname', 'venders.name as vname')
            //     ->whereBetween('purchases.created_at', [$startDate, $endDate])
            //     ->where('purchases.vender_id', '=', $request->vendor)
            //     ->first();
            // dd($purdata);


            return view('report.daily_purchase', compact('warehouse', 'vendor', 'arrDuration', 'data', 'filter', 'allPurchases', 'purdata',));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function salesReportExport(Request $request)
    {
        ...
        if ($request->report == '#item') {
            ...
            $invoiceItems = Invoice::select(
                'customers.name',
                \DB::raw('count(invoices.customer_id) as invoice_count'),
                \DB::raw('sum(invoice_products.price) as price'),
                \DB::raw('sum((invoice_products.price) + (invoice_products.price * (taxes.rate / 100 ))) as total_tax')
            );

            $invoiceItems->leftJoin('customers', 'customers.id', 'invoices.customer_id');
            $invoiceItems->leftJoin('invoice_products', 'invoice_products.invoice_id', 'invoices.id');
            $invoiceItems->leftJoin('taxes', \DB::raw('FIND_IN_SET(taxes.id, invoice_products.tax)'), '>', \DB::raw('0'));
            $invoiceItems->where('invoices.created_by', \Auth::user()->creatorId());
            $invoiceItems->where('invoices.created_at', '>=', $start);
            $invoiceItems->where('invoices.created_at', '<=', $end);
            $invoiceItems->groupBy('invoices.customer_id');
            $invoiceItems = $invoiceItems->get()->toArray();
    ...
    public function ReceivablesReport(Request $request)
    {
        ...
        $invoiceCustomers = Invoice::select(
            'customers.name',
            \DB::raw('sum((invoice_products.price) + (invoice_products.price * (taxes.rate / 100 ))) as total_price'),
            \DB::raw('sum((invoice_payments.amount)) as pay_price'),
            \DB::raw('sum((credit_notes.amount)) as credit_price')
        );
        ...
        return view('report.receivable_report', compact('filter', 'invoiceCustomers'));
    }
    ...
    public function ReceivablesExport(Request $request)
    {
        ...
        $data = Excel::download(new ReceivableExport($invoiceCustomers, $start, $end, $companyName), $name . '.xlsx');
        ob_end_clean();
    ...
    public function ReceivablesPrint(Request $request)
    {
        ...
        $invoiceCustomers = Invoice::select(
            'customers.name',
            \DB::raw('sum((invoice_products.price * invoice_products.quantity) + (invoice_products.price * (taxes.rate / 100 ))) as total_price'),
            \DB::raw('sum((invoice_payments.amount)) as pay_price'),
            \DB::raw('sum((credit_notes.amount)) as credit_price')
        );
