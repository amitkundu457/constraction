    ...
    <section class="w-full h-full py-8 relative px-14 flex">
        ...
        <div class="w-4/5 px-4">
            <div class="border border-gray-300 border-t-0 p-2 text-sm font-semibold text-gray-500">
                <h1 class="px-3">Properties Wanted ({{ $wanteds->count() }})</h1>
            </div>
                        ...
                        @foreach ($wanteds as $w)
                            <div class="px-5 border-b border-gray-300 border-t-0 py-3">
                                <a href="{{ route('property.details', $w->id) }}"
                                    class="text-[#59C517] font-bold hover:underline">{{ $w->title }}</a>
                                <div class="flex">
                                    <div class="py-3">
                                        <img width="200"
                                            src="{{ $w->photo ? asset('photo/' . $w->photo) : 'https://www.its.ac.id/tmesin/wp-content/uploads/sites/22/2022/07/no-image.png' }}"
                                            alt="">
                                    </div>
                                    <div class="px-3 py-3">
                                        <p class="font-semibold bg-gray-400 px-4 py-1 rounded text-white w-max">Starting
                                            From
                                            &#8377; {{ number_format($w->price) }}/-</p>
