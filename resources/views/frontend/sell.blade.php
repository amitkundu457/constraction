...
@section('content')
    <section class="w-full h-full py-8 relative px-14 flex">
        <aside class="w-1/5 h-full">
            ...
            <form action="{{ url('/sell') }}" class="h-full border border-gray-400 border-t-0 space-y-2 py-2">
        ...
        <div class="w-4/5 px-4">
            <div class="border border-gray-300 border-t-0 p-2 text-sm font-semibold text-gray-500">
                <h1 class="px-3">Properties For Sale ({{ $sells->count() }})</h1>
            </div>
                    ...
                    @if ($sells->count() > 0)
                        @foreach ($sells as $s)
                            <div class="px-5 border-b border-gray-300 border-t-0 py-3">
                                <a href="{{ route('property.details', $s->id) }}"
                                    class="text-[#59C517] font-bold hover:underline">{{ $s->title }}</a>
                                <div class="flex">
                                    <div class="py-3">
                                        <img width="200"
                                            src="{{ $s->photo ? asset('photo/' . $s->photo) : 'https://www.its.ac.id/tmesin/wp-content/uploads/sites/22/2022/07/no-image.png' }}"
                                            alt="">
                                    </div>
                                    <div class="px-3 py-3">
                                        <p class="font-semibold bg-gray-400 px-4 py-1 rounded text-white w-max">Starting
                                            From
                                            &#8377; {{ number_format($s->price) }}/-</p>
                                            <p class="text-sm flex items-center text-gray-500 gap-x-1 mt-2">
                                                <i class="fas fa-map-marker-alt"></i>
                                                <span>{{ $s->location }}</span>
                                            </p>
                                        <div class="pt-1 flex items-center flex-wrap">
                                            @foreach ($s->amenities as $a)
