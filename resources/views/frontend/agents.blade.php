...
@section('content')
    <section class="w-full h-full py-8 relative px-14 flex justify-center">
        {{-- <aside class="w-1/5 h-full">
            <div class="bg-[#59C517] text-center py-2 font-semibold text-white">
                <h1 class="">Refine Your search</h1>
            </div>
            <form action="{{ url('/wanted') }}" class="h-full border border-gray-400 border-t-0 space-y-2 py-2">
                <div class="px-3 space-y-2">
                    <label for="" class="text-sm font-semibold">Location</label>
                    <input type="text" name="location" value="{{ request()->query('location') }}"
                        placeholder="Type location" class="p-2 border border-gray-300 rounded text-sm w-full">
                </div>
                <div class="px-3 space-y-2">
                    ...
                    <select name="property_type" id="" class="p-2 border border-gray-300 rounded text-sm w-full">
                        ...
                        @foreach (\App\Models\Contruct::all() as $item)
                            <option value="{{ $item->contruct_name }}" @selected(request()->query('property_type') == $item->contruct_name)>
                                {{ $item->contruct_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="px-3 space-y-2">
        ...
        <div class="w-4/5 px-4">
            <div class="border border-gray-300 border-t-0 p-2 text-sm font-semibold text-gray-500">
                <h1 class="px-3">Agents ({{ $agents->count() }})</h1>
            </div>
            <div class="border border-gray-300 border-y-0">
                    ...
                    @if ($agents->count() > 0)
                        @foreach ($agents as $a)
                            <div class="px-5 border-b border-gray-300 border-t-0 py-3">
                                <a href="#"
                                    class="text-[#59C517] font-bold hover:underline">{{ $a->name }}</a>
                                <div class="flex">
                                    <div class="py-3">
                                        <img width="200"
                                    ...
                                    <div class="px-3 py-3">
                                        <p class="font-semibold text-gray-500">Member Since {{ Carbon\Carbon::parse($a->created_at)->format('d M, Y') }}</p>
                                        {{-- <p class="font-semibold bg-gray-400 px-4 py-1 rounded text-white w-max"></p> --}}
                                        <div class="border-y border-gray-500 py-3 space-y-2">
                                            <p class="text-xs font-semibold text-">Phone : {{ $a->contact }}</p>
                                            <p class="text-xs font-semibold text-">Email : {{ $a->email }}</p>
                                        </div>
                                        <div class="py-2 flex items-center flex-wrap">
                                            {{-- @foreach ($w->amenities as $a)
                                                <div class="text-sm font-semibold text-[#59C517] p-1 space-x-1">
                                                    <i class="fas fa-check"></i>
                                                    <span
                                                        class="text-gray-900">{{ \App\Models\PropertyAmenity::findOrFail($a)->name }}</span>
                                                </div>
                                            @endforeach --}}
                                        </div>
                                        <div class="flex items-center gap-2">
                                            {{-- <a href="{{ route('property.details', $w->id) }}"
                ...
                @if ($agents->lastPage() != 1)
                    <div
                        class="flex justify-center items-center py-4 border border-b border-gray-300 gap-2 [&>a]:block [&>a]:font-semibold [&>a]:text-white [&>a]:px-2 [&>a]:py-0.5  [&>a]:rounded-full [&>a]:bg-[#59C517]">
