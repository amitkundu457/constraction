{{ Form::open(['url' => ['leads',$lead->id],'method'=>'put']) }}
<div class="modal-body">
    ...
    @endif
    {{-- end for ai module --}}
    <div class="row" x-data="{ type: '{{ strtolower(\App\Models\Source::findOrFail($lead->source)->name) }}', ltype: '{{ $lead->lead_for }}' }">
        ...
        <div class="col-6 form-group">
            ...
            {{ Form::text('name', $lead->name, ['class' => 'form-control', 'required' => 'required']) }}
        </div>
        <div class="col-6 form-group">
            {{ Form::label('email', __('Email'), ['class' => 'form-label']) }}
            {{ Form::text('email', $lead->email, ['class' => 'form-control', 'required' => 'required']) }}
        </div>
        <div class="col-6 form-group">
            {{ Form::label('phone', __('Phone'), ['class' => 'form-label']) }}
            {{ Form::text('phone', $lead->phone, ['class' => 'form-control', 'required' => 'required']) }}
        </div>
        <div class="col-6 form-group">
            ...
            <select name="source_id" id="" class="form-control"
                ...
                @foreach ($sources as $source)
                    <option value="{{ $source->id }}" @selected($lead->source == $source->id) data-title="{{ strtolower($source->name) }}">
                        {{ $source->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="col-6 form-group" x-cloak x-show="type === 'website' ||type === 'websites'">
            ...
            <input type="text" name="link" class="form-control" value="{{ $lead->link }}">
        </div>
        <div class="col-6 form-group" x-cloak x-show="type === 'social media'">
            {{ Form::label('smedia', __('Social Media Names'), ['class' => 'form-label']) }}
           <input type="text" name="smedia" class="form-control" value="{{ $lead->socials }}">
        </div>
        <div class="col-6 form-group">
            ...
            <select name="lead_for" id="" class="form-control"
                ...
                <option value="plot" data-title="plot" @selected($lead->lead_for == 'plot')>Plot</option>
                <option value="property" data-title="property"  @selected($lead->lead_for == 'property')>Property</option>
        ...
        <div class="col-6 form-group" x-cloak x-show="ltype === 'property'">
            ...
            <select name="property_id" id="" class="form-control">
                ...
                @foreach ($properties as $property)
                    <option value="{{ $property->id }}" @selected($lead->property_id == $property->id)>{{ $property->title }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-6 form-group" x-cloak x-show="ltype === 'plot'">
            ...
            <select name="plot_id" id="" class="form-control">
                ...
                @foreach ($plots as $plot)
                    <option value="{{ $plot->id }}" @selected($lead->plot_id == $plot->id)>{{ $plot->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-6 form-group">
            {{ Form::label('followup', __('Follow Up'), ['class' => 'form-label']) }}
            {{ Form::date('followup', $lead->date, ['class' => 'form-control', 'required' => 'required']) }}
        </div>
        <div class="col-6 form-group">
        ...
        <div class="col-6 form-group">
            ...
            <select name="stage" id="status" class="form-control" required>
                ...
                @foreach ($status as $stat)
                    <option value="{{ $stat->id }}" @selected($lead->pipeline_id == $stat->id)>{{ $stat->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-12 form-group">
            {{ Form::label('Comments', __('Comment / Remarks'), ['class' => 'form-label']) }}
            <textarea required name="subject" id="" cols="30" rows="8" class="form-control">{{ $lead->subject }}</textarea>
        </div>
    </div>
</div>
