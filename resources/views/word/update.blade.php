...
@section('content')
    <form action="{{ route('word.edit', $word->id) }}" method="POST" enctype="multipart/form-data" id="word">
        @csrf
        <div class="row">
                ...
                <div class="d-flex justify-content-between align-items-center px-5 my-2" x-data="loadPreview()">
                    <div class="d-flex flex-column align-items-center px-3">
                        <input type="file" x-on:change="change($event,'left')" name="sig_sender" class="d-none"
                            id="left">
                            <input type="text" class="form-control" name="ltext" placeholder="enter text" value="{{ $word->ltext }}">
                    ...
                    <div class="d-flex flex-column align-items-center px-3">
                        <input type="file" x-on:change="change($event,'right')" name="sig_receiver" class="d-none"
                            id="right">
                            <input type="text" class="form-control" name="rtext" placeholder="enter text" value="{{ $word->rtext }}">
...
@push('script')
    ...
    <script>
        ...
        function loadPreview() {
            return {
                limage: @json($word->sig_sender ? Storage::disk('public')->url($word->sig_sender) : null),
                rimage: @json($word->sig_receiver ? Storage::disk('public')->url($word->sig_receiver) : null),
