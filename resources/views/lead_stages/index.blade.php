                    ...
                    <div class="card-body">
                        <div class="tab-content" id="pills-tabContent">
                            ...
                            @foreach($stages as $key => $stage)
                                <div class="tab-pane fade show active" id="tab{{$key}}" role="tabpanel" aria-labelledby="pills-user-tab-1">
                                        ...
                                        {{-- @foreach ($pipeline['lead_stages'] as $lead_stages) --}}
                                            <li class="d-flex align-items-center justify-content-between list-group-item" data-id="{{$stage->id}}">
                                                ...
                                                <span class="float-end">
                                                    @can('edit lead stage')
                                                        <div class="action-btn bg-info ms-2"><a href="#" class="mx-3 btn btn-sm d-inline-flex align-items-center" data-url="{{ URL::to('lead_stages/'.$stage->id.'/edit') }}" data-ajax-popup="true" data-size="md" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-title="{{__('Edit Lead Stages')}}">
                                                    ...
                                                    {{-- @if(count($pipeline['lead_stages'])) --}}
                                                        @can('delete lead stage')
                                                            <div class="action-btn bg-danger ms-2">
                                                                {!! Form::open(['method' => 'DELETE', 'route' => ['lead_stages.destroy', $stage->id]]) !!}
